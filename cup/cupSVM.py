...
def nested_grid_search_kfold(train_path, test_path, results_file):
    ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    processor = DatasetProcessor(ROOT_DIR)
    x_train, y_train, x_val, y_val = processor.read_tr(split=True)
    for i, target in enumerate(['TARGET_x', 'TARGET_y', 'TARGET_z']):
        y_train_target = y_train[:, i]
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(outer_cv.split(x_train, y_train_target), start=1):
            X_train_fold, X_val_fold = x_train[train_idx], x_val[val_idx]
            y_train_fold, y_val_fold = y_train_target[train_idx], y_val_target[val_idx]
            ...
            print(f"Migliori parametri per {target}:{best_params_}") 
            ...
            predictions[target] = y_pred
            print(f"Metriche per {target}:")
            print(f"MAE: {mean_absolute_error(y_val_fold, y_pred)}")
            print(f"MSE: {mean_squared_error(y_val_fold, y_pred)}")
