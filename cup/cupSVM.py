...
def nested_grid_search_kfold():
    ...
    for i, target in enumerate(['TARGET_x', 'TARGET_y', 'TARGET_z']):
        ...
        # Create a figure for this target's plots
        ...
        plt.subplot(2, 2, 3)
        errors = np.abs(y_test_pred - y_test_target)  # Using absolute error for MEE
        sns.histplot(errors, kde=True)
        plt.title('Absolute Prediction Error Distribution')
        plt.xlabel('Absolute Error')
        
        # Plot 4: Standard Deviation of Predictions
        plt.subplot(2, 2, 4)
        plt.hist(std_predictions, bins=30)
